...
<svelte:head>
  <title>Settings · Unit Conversion</title>
</svelte:head>
...
<div class="space-y-6">
  <div class="flex items-center gap-3">
    <a
      href="{base}/"
      class="flex items-center justify-center rounded-lg p-1.5 text-slate-400 transition-colors hover:text-white"
      aria-label="Back"
    >
      <ArrowLeft size={20} />
    </a>
        ...
        {#each unitSystems as sys}
          ...
          >
            ...
            {#if $settings.unitSystem === sys.value}
              <span class="h-2 w-2 flex-shrink-0 rounded-full bg-sky-400">
              </span>
            {/if}
          </button>
        {/each}
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-slate-500">
            Cache app for use without internet
          </p>
        </div>
        <button
          role="switch"
